 ...
 * Time: 下午2:08
 */
namespace Home\Services\Alipay;
Vendor('AlipayWapPay.AopSdk');
Vendor('AlipayWapPay.aop.request.AlipayTradeWapPayRequest');
Vendor('AlipayWapPay.aop.AopClient.php');
class Alipay{
    //支付宝网关地址
    ...
    function __construct($alipay_config){
        ...
        if(empty($this->appid)||trim($this->appid)==""){
            throw_exception("appid should not be NULL!");
        }
        if(empty($this->private_key)||trim($this->private_key)==""){
            throw_exception("private_key should not be NULL!");
        }
        if(empty($this->alipay_public_key)||trim($this->alipay_public_key)==""){
            throw_exception("alipay_public_key should not be NULL!");
        }
        if(empty($this->charset)||trim($this->charset)==""){
            throw_exception("charset should not be NULL!");
        }
        if(empty($this->gateway_url)||trim($this->gateway_url)==""){
            throw_exception("gateway_url should not be NULL!");
        }
